    ...
    <div className="pb-12">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 flex flex-col md:flex-row items-center gap-6 mb-8">
        ...
        <div className="flex-1 flex flex-col gap-2 items-center md:items-start">
          <h2 className="text-2xl font-bold flex items-center gap-2">{propUserData.name || 'Student Name'}
            {propUserData.verifiedOn && <Check size={18} className="text-green-500" />}
          </h2>
          <div className="text-gray-600 flex flex-wrap gap-4">
            <span><span className="font-medium">Roll No:</span> {propUserData.rollNumber || '-'}</span>
            <span><span className="font-medium">Program:</span> {propUserData.program || '-'}</span>
            <span><span className="font-medium">Batch:</span> {propUserData.batch || '-'}</span>
          </div>
          <div className="flex gap-2 mt-2">
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs">{propUserData.careerPath}</span>
            <span className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-xs">{propUserData.placementCycle || 'N/A'}</span>
          </div>
        </div>
